...
<body>
    ...
    <ul>
        ...
        <li>
            <a href="<?= url(htmlspecialchars($file['file_path'])) ?>" target="_blank"><?= htmlspecialchars($file['file_path']) ?></a>
            <?php if ($ticket['user_id'] == $_SESSION['user']['id'] || $_SESSION['user']['role'] === 'admin'): ?>
            <form action="<?= url('/attachments/' . $file['id'] . '/delete') ?>" method="POST" style="display:inline" onsubmit="return confirm('Hapus lampiran ini?')">
        <?= csrfField() ?>
    ...
    <ul>
        ...
        <?= csrfField() ?>
                        <textarea name="message" rows="2" required><?= htmlspecialchars($comment['message']) ?></textarea><br>
                        <button type="submit">Simpan</button>
                    </form>
                </details>
            <?php endif; ?>
            <?php if ($comment['user_id'] == $_SESSION['user']['id'] || $_SESSION['user']['role'] === 'admin'): ?>
                <form action="<?= url('/comments/' . $comment['id'] . '/delete') ?>" method="POST" style="display:inline" onsubmit="return confirm('Hapus komentar ini?')">
        <?= csrfField() ?>
                    <button type="submit">Hapus</button>
                </form>
            <?php endif; ?>
    ...
    <p><a href="<?= url($backUrl) ?>">Kembali</a> | <a href="<?= url('/tickets/' . $ticket['id'] . '/export-pdf') ?>">Cetak PDF</a></p>
</body>
</html>
